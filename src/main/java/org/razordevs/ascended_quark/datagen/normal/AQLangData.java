...
public class AQLangData extends NitrogenLanguageProvider {
	...
	protected void addTranslations() {
		...
		this.add("lore.item.ascended_quark.ambrosium_torch_arrow",
				"An arrow tied to an Ambrosium Torch. It imbues the healing properties of Ambrosium, and places the torch on impact.");
		this.add("lore.item.ascended_quark.blue_swet_in_a_bucket",
				"Seems like you caught a Blue Swet! Not that useful, but it's an easy way to transport them. They might get excited too!");
		this.add("lore.item.ascended_quark.blue_swet_in_a_skyroot_bucket",
				"Seems like you caught a Blue Swet! Not that useful, but it's an easy way to transport them. They might get excited too!");
		this.add("lore.item.ascended_quark.golden_swet_in_a_bucket",
				"Seems like you caught a Golden Swet! Not that useful, but it's an easy way to transport them. They might get excited too!");
		this.add("lore.item.ascended_quark.golden_swet_in_a_skyroot_bucket",
				"Seems like you caught a Golden Swet! Not that useful, but it's an easy way to transport them. They might get excited too!");
		this.add("lore.item.ascended_quark.quicksoil_glass_shard",
				"Apparently something broke. You can still use some of the shards to craft the glass again.");

		this.add("lore.block.ascended_quark.holystone_furnace",
				"A versatile furnace that can also use Ambrosium as fuel. Also looks fancier in white.");
		this.add("lore.block.ascended_quark.ambrosium_lamp",
				"A lamp made out of Holystone and Ambrosium. Ambrosium Blocks can increase its light up to 4 times.");
		this.add("lore.block.ascended_quark.blue_berry_crate",
				"A handy crate for storing blue berries! A very good decoration block!");
		this.add("lore.block.ascended_quark.goldenleaf_berries_crate",
				"A handy crate for storing Goldenleaf berries! A very good decoration block!");
		this.add("lore.block.ascended_quark.quicksoil_framed_glass",
				"Combine Iron Ingots and Quicksoil Glass, and you get yourself some fancy Quicksoil Framed Glass. Great for some more rustic or medieval builds.");
		this.add("lore.block.ascended_quark.quicksoil_framed_glass_pane",
	...
	private void addMenuLangComponents() {
		...
		this.add(AscendedQuark.MODID + ".gui.config.social.bluesky", "RazorDevs Bluesky");
	}

	private void addLadderLore(Block block) {
		this.add("lore." + block.getDescriptionId(), "Ladders made out of " + clearBlockOrigin(block, 7)
				+ "! Much better decoration than plain ol' stairs...");
	}

	private void addHollowLogLore(Block block) {
		this.add("lore." + block.getDescriptionId(), "Can be crafted with four " + clearBlockOrigin(block, 7)
				+ " Logs. You can sneak into them horizontally, and climb inside of them vertically!");
	}

	private void addBricksLore(Block block) {
		this.add("lore." + block.getDescriptionId(),
				"Used as a building material native to the Aether. It is made from " + clearBlockOrigin(block, 7)
						+ " and is sturdier than it too.");
	}
	...
	private void addVerticalSlabLore(Block block) {
		this.add("lore." + block.getDescriptionId(), "Crafted from " + clearBlockOrigin(block, 14)
				+ ". Vertical Slabs are half blocks, versatile for decoration. Try adding some to a building's window!");
	}
	private void addStairsLore(Block block) {
		this.add("lore." + block.getDescriptionId(), "Crafted from " + clearBlockOrigin(block, 7)
	...
	private void addWallLore(Block block) {
		this.add("lore." + block.getDescriptionId(), "Crafted from " + clearBlockOrigin(block, 5)
				+ ". Can be used for decorative enclosures and defenses. Great for keeping nasty intruders away!");
	}
	private void addHedgeLore(Block block) {
		this.add("lore." + block.getDescriptionId(), "Crafted from " + clearBlockOrigin(block, 6)
				+ ". They connect to each other like fences, and have the same bounding box. You can even put flowers on them!");
	}
	private void addPostLore(Block block) {
		this.add("lore." + block.getDescriptionId(), "Crafted from " + clearBlockOrigin(block, 5)
				+ ". These are slim log style blocks you can lay down vertically or horizontally. Chains and lanterns can also connect to them.");
	}
	private void addCarpetLore(Block block) {
		this.add("lore." + block.getDescriptionId(), "Crafted from " + clearBlockOrigin(block, 7)
				+ ". Leaf Carpets are a great way to add natural elements to your builds!");
	}
	...
	/**
	 * Returns the block of origin, cleaning the string and adding uppercase letters
	 * from the descriptionId of the passed block. Used in the Lang datageneration
	 * for various Blocks lore.
	 *
	 * @param block
	 *            Block to elaborate.
	 *
	 * @param endIndex
	 *            Index position used to exclude the suffix depending on the type of
	 *            block.
	 */
	private String clearBlockOrigin(Block block, int endIndex) {
		return StringUtils.capitaliseAllWords(
				block.getDescriptionId().substring(21, block.getDescriptionId().length() - endIndex).replace("_", " "));
	}
