...
public class AQItemTagProvider extends ItemTagsProvider {
	...
	public AQItemTagProvider(PackOutput p_275343_, CompletableFuture<HolderLookup.Provider> p_275729_,
	...
	public static final String[] Wood = {"skyroot", "roseroot", "yagroot", "cruderoot", "sunroot", "conberry"};
	...
	protected void addTags(HolderLookup.Provider provider) {
		...
		blocks.forEach(block -> {
			if (block instanceof AQHollowLogBlock)
				tag(ItemTags.create(ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "hollow_logs")))
						.add(block.asItem());
		});
		...
		blocks.forEach(block -> {
			if (block instanceof ChestBlock) {
				...
				if ((block instanceof AQTrappedVariantChestBlock)) {
					...
					tag(ItemTags.create(ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "revertable_chests")))
							.add(block.asItem());
					tag(ItemTags.create(ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "boatable_chests")))
							.add(block.asItem());
				}
			}
		});
		...
		blocks.forEach(block -> {
			if (block instanceof LadderBlock)
				tag(ItemTags.create(ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "ladders")))
						.add(block.asItem());
		});

		blockMap.keySet().forEach(s -> {
			if (blockMap.get(s) instanceof VerticalSlabBlock block) {
				tag(ItemTags.create(ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "vertical_slabs")))
						.add(block.asItem());
