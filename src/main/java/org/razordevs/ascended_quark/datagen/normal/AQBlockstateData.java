...
public class AQBlockstateData extends AetherBlockStateProvider {
	...
	public void registerStatesAndModels() {
		...
		this.hedge("decorated_holiday_skyroot", AetherBlocks.DECORATED_HOLIDAY_LEAVES.get(),
				AetherBlocks.SKYROOT_LOG.get(), "natural/");
		this.hedge("crystal_skyroot", AetherBlocks.CRYSTAL_LEAVES.get(), AetherBlocks.SKYROOT_LOG.get(), "natural/");
		this.hedge("crystal_fruit_skyroot", AetherBlocks.CRYSTAL_FRUIT_LEAVES.get(), AetherBlocks.SKYROOT_LOG.get(),
				"natural/");
	...
	public void ladder(String type) {
		...
		ModelFile ladder = this.models().withExistingParent(this.name(block), this.mcLoc("block/block"))
				.renderType(ResourceLocation.withDefaultNamespace("cutout")).ao(false).texture("particle", location)
				.texture("texture", location).element().from(0.0F, 0.0F, 15.2F).to(16.0F, 16.0F, 15.2F).shade(false)
				.face(Direction.NORTH).uvs(0.0F, 0.0F, 16.0F, 16.0F).texture("#texture").end().face(Direction.SOUTH)
				.uvs(16.0F, 0.0F, 0.0F, 16.0F).texture("#texture").end().end();
		this.getVariantBuilder(block).forAllStatesExcept((state) -> {
			Direction direction = state.getValue(LadderBlock.FACING);
			return ConfiguredModel.builder().modelFile(ladder).rotationY((int) (direction.toYRot() + 180.0F) % 360)
					.build();
	...
	public void chest(String type, Block planks, String location) {
		...
		this.getVariantBuilder(block).partialState().addModels(new ConfiguredModel(
				this.chest(this.name(block), this.texture(BuiltInRegistries.BLOCK.getKey(planks), location))));
		this.getVariantBuilder(trapped).partialState().addModels(new ConfiguredModel(
				this.chest(this.name(trapped), this.texture(BuiltInRegistries.BLOCK.getKey(planks), location))));
	}
	...
	public void chest(String type, Block planks) {
		...
		this.getVariantBuilder(trapped).partialState().addModels(new ConfiguredModel(
				this.chest(this.name(trapped), this.texture(BuiltInRegistries.BLOCK.getKey(planks)))));
	}
	...
	public void pillar(RotatedPillarBlock block) {
		this.axisBlock(block, this.extend(this.texture(this.name(block)), "_side"),
	...
	public void postBlock(Block block, ModelFile post) {
		...
		ModelFile small = this.models()
				.getExistingFile(ResourceLocation.fromNamespaceAndPath(AscendedQuark.MODID, "block/chain_small"));
		ModelFile small_up = this.models()
				.getExistingFile(ResourceLocation.fromNamespaceAndPath(AscendedQuark.MODID, "block/chain_small_top"));

		builder.part().modelFile(post).addModel().condition(AQWoodenPostBlock.AXIS, Direction.Axis.Y);
		builder.part().modelFile(post).rotationY(90).rotationX(90).addModel().condition(AQWoodenPostBlock.AXIS,
				Direction.Axis.X);
		...
		builder.part().modelFile(small).rotationX(90).rotationY(90).addModel().condition(AQWoodenPostBlock.CHAINED[4],
				true);
		builder.part().modelFile(small_up).rotationX(90).rotationY(90).addModel()
				.condition(AQWoodenPostBlock.CHAINED[5], true);
	}

	public void hedge(String type, Block leaf, Block log) {
		this.hedgeBlock(blockMap.get(type + "_hedge"), this.texture(BuiltInRegistries.BLOCK.getKey(leaf)),
				this.texture(BuiltInRegistries.BLOCK.getKey(log)));
	}

	public void hedge(String type, Block leaf, Block log, String location) {
		this.hedgeBlock(blockMap.get(type + "_hedge"), this.texture(BuiltInRegistries.BLOCK.getKey(leaf), location),
				this.texture(BuiltInRegistries.BLOCK.getKey(log), location));
	}

	public void hedgeBlock(Block block, ResourceLocation leaf, ResourceLocation log) {
		String baseName = this.key(block).toString();
		this.fourWayBlockExtended(block, this.hedgePost(baseName + "_post", leaf, log),
				this.hedgeSide(baseName + "_side", leaf), this.hedgeExtend(baseName + "_extend", leaf));
	}

	public ModelFile hedgePost(String name, ResourceLocation leaves, ResourceLocation log) {
		return this.models()
				.withExistingParent(name, ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "block/hedge_post"))
				.texture("log", log).texture("leaf", leaves).renderType("cutout");
	}

	public ModelFile hedgeSide(String name, ResourceLocation leaves) {
		return this.models()
				.withExistingParent(name, ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "block/hedge_side"))
				.texture("leaf", leaves).renderType("cutout");
	}

	public ModelFile hedgeExtend(String name, ResourceLocation leaves) {
		return this.models()
				.withExistingParent(name, ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "block/hedge_extend"))
				.texture("leaf", leaves).renderType("cutout");
	}
	...
	public ModelFile cubeAllTranslucent(Block block) {
		return this.models().cubeAll(this.name(block), this.texture(this.name(block)))
	...
	public void hollowLog(String type, Block log) {
		...
		this.hollowLog((RotatedPillarBlock) block, this.texture(BuiltInRegistries.BLOCK.getKey(log)),
	...
	public void hollowLog(String type, Block log, Block stripped, String prefix) {
		Block block = blockMap.get("hollow_" + type + "_log");
		this.hollowLog((RotatedPillarBlock) block, this.texture(BuiltInRegistries.BLOCK.getKey(log), prefix),
	...
	public void hollowLog(RotatedPillarBlock block, ResourceLocation side, ResourceLocation end,
	...
	public ModelFile hollowCubeColumn(String name, ResourceLocation side, ResourceLocation end,
			ResourceLocation inside) {
		return this.models()
				.withExistingParent(name, ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "block/hollow_log"))
				.texture("side", side).texture("end", end).texture("inside", inside)
				.renderType(ResourceLocation.withDefaultNamespace("translucent"));
	}
	...
	public ModelFile cubeColumnHorizontal(String name, ResourceLocation side, ResourceLocation end,
		...
		return this.models()
				.withExistingParent(name,
						ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "block/hollow_log_horizontal"))
				.texture("side", side).texture("end", end).texture("inside", inside)
				.renderType(ResourceLocation.withDefaultNamespace("translucent"));
	}
	...
	public void compressed(String type) {
		...
		ModelFile compressed = this.cubeBottomTop(this.name(block), this.texture(this.name(block)),
				AscendedQuark.asResource("block/crate_bottom"), this.extend(this.texture(this.name(block)), "_top"));
	...
	public void bookshelf(String type, Block planks) {
		...
		ModelFile bookshelf = this.cubeBottomTop(this.name(block), this.texture(this.name(block)),
	...
	public void verticalSlab(String type, Block texture) {
		...
		this.verticalSlabBlock(block, this.verticalSlab(type + "_vertical_slab", this.blockTexture(texture)),
				this.models().getExistingFile(this.texture(BuiltInRegistries.BLOCK.getKey(texture))));
	}
	...
	public void verticalSlab(String type, String texture) {
		Block block = blockMap.get(type + "_vertical_slab");
		this.verticalSlabBlock(block, this.verticalSlab(type + "_vertical_slab", this.texture(texture)),
				this.models().getExistingFile(this.texture(texture)));
	}

	public void verticalSlab(String type, Block texture, String suffix) {
		Block block = blockMap.get(type + "_vertical_slab");
		this.verticalSlabBlock(block,
				this.verticalSlab(type + "_vertical_slab",
						this.texture(BuiltInRegistries.BLOCK.getKey(texture), suffix)),
				this.models().getExistingFile(this.texture(BuiltInRegistries.BLOCK.getKey(texture))));
	}

	public void verticalSlabBlock(Block block, ModelFile halfSlab, ModelFile fullSlab) {
		this.getVariantBuilder(block).partialState()
				.with(VerticalSlabBlock.TYPE, VerticalSlabBlock.VerticalSlabType.NORTH).modelForState()
				.modelFile(halfSlab).rotationY(0).uvLock(true).addModel()

				.partialState().with(VerticalSlabBlock.TYPE, VerticalSlabBlock.VerticalSlabType.SOUTH).modelForState()
				.modelFile(halfSlab).rotationY(180).uvLock(true).addModel()

				.partialState().with(VerticalSlabBlock.TYPE, VerticalSlabBlock.VerticalSlabType.EAST).modelForState()
				.modelFile(halfSlab).rotationY(90).uvLock(true).addModel()

				.partialState().with(VerticalSlabBlock.TYPE, VerticalSlabBlock.VerticalSlabType.WEST).modelForState()
				.modelFile(halfSlab).rotationY(270).uvLock(true).addModel()

				.partialState().with(VerticalSlabBlock.TYPE, VerticalSlabBlock.VerticalSlabType.DOUBLE).modelForState()
				.modelFile(fullSlab).addModel();
	}
	...
	public void verticalPlank(String type, Block texture, String suffix) {
		Block block = blockMap.get("vertical_" + type + "_planks");
		this.simpleBlock(block, this.verticalPlank("vertical_" + type + "_planks",
				this.texture(BuiltInRegistries.BLOCK.getKey(texture), suffix)));
	}

	public ModelFile verticalPlank(String name, ResourceLocation texture) {
		return this.models().singleTexture(name,
				ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "block/vertical_planks"), "all", texture);
	}
	...
	public void leafCarpet(String type, Block texture, String suffix) {
		...
		this.simpleBlock(block,
				this.leafCarpet(type + "_leaf_carpet", this.texture(BuiltInRegistries.BLOCK.getKey(texture), suffix)));
	}

	public ModelFile leafCarpet(String name, ResourceLocation texture) {
		return this.models().singleTexture(name,
				ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "block/leaf_carpet"), "all", texture)
				.renderType("cutout");
	}

	public ModelFile verticalSlab(String name, ResourceLocation texture) {
		return this.sideBottomTop(name, ResourceLocation.fromNamespaceAndPath(Quark.MOD_ID, "block/vertical_slab"),
				texture);
	}

	public ModelFile sideBottomTop(String name, ResourceLocation parent, ResourceLocation location) {
		return this.models().withExistingParent(name, parent).texture("side", location).texture("bottom", location)
				.texture("top", location);
	}

	public void pottedPlant(Block block, Block flower) {
		ModelFile pot = this.models().withExistingParent(this.name(block), this.mcLoc("block/flower_pot_cross"))
	...
	public void saplingBlock(Block block) {
		ModelFile sapling = models().cross(this.name(block), this.texture(this.name(block)))
				.renderType(ResourceLocation.withDefaultNamespace("cutout"));
		this.getVariantBuilder(block).forAllStatesExcept(state -> ConfiguredModel.builder().modelFile(sapling).build(),
				SaplingBlock.STAGE);
	}
	...
	public void verticalSlabBlock(Block block, Block baseBlock) {
		this.verticalSlabBlock((VerticalSlabBlock) block, this.texture(this.name(baseBlock)),
				this.texture(this.name(baseBlock)));
	}
	...
	public void verticalSlabBlock(VerticalSlabBlock block, ResourceLocation doubleslab, ResourceLocation side,
			ResourceLocation bottom, ResourceLocation top) {
		ModelFile vertical_slab = this.models()
				.withExistingParent(this.name(block),
						ResourceLocation.fromNamespaceAndPath("quark", "block/vertical_slab"))
	...
	public void blockDoubleDrops(Block block) {
		this.getVariantBuilder(block).forAllStatesExcept(
				state -> ConfiguredModel.builder().modelFile(this.cubeAll(block)).build(),
	...
	protected BlockModelBuilder makeWallPostModel(int width, int height, String name) {
		return models().withExistingParent(name, this.mcLoc("block/block")).element().from(8 - width, 0.0F, 8 - width)
				.to(8 + width, height, 8 + width).face(Direction.DOWN).texture("#top").cullface(Direction.DOWN).end()
				.face(Direction.UP).texture("#top").cullface(Direction.UP).end().face(Direction.NORTH).texture("#side")
				.end().face(Direction.SOUTH).texture("#side").end().face(Direction.WEST).texture("#side").end()
				.face(Direction.EAST).texture("#side").end().end();
	}

	protected BlockModelBuilder makeWallSideModel(int length, int height, String name,
			ModelBuilder.FaceRotation faceRotation, int u1, int u2) {
		return models().withExistingParent(name, this.mcLoc("block/block")).element().from(5.0F, 0.0F, 0.0F)
				.to(11.0F, height, length).face(Direction.DOWN).texture("#top").rotation(faceRotation)
				.uvs(u1, 5, u2, 11).cullface(Direction.DOWN).end().face(Direction.UP).texture("#top")
				.rotation(faceRotation).uvs(u1, 5, u2, 11).end().face(Direction.NORTH).texture("#side")
				.cullface(Direction.NORTH).end().face(Direction.SOUTH).texture("#side").end().face(Direction.WEST)
				.texture("#side").end().face(Direction.EAST).texture("#side").end().end();
	}

	public void logWallBlock(WallBlock block, Block baseBlock, String location, String modid, boolean postUsesTop,
			ModelFile postBig, ModelFile postShort, ModelFile postTall, ModelFile side, ModelFile sideAlt,
			ModelFile sideTall, ModelFile sideTallAlt, ModelFile sideShort, ModelFile sideAltShort,
			ModelFile sideTallShort, ModelFile sideTallAltShort) {
		this.logWallBlockInternal(block, this.name(block),
				ResourceLocation.fromNamespaceAndPath(modid, "block/" + location + this.name(baseBlock)), postUsesTop,
				postBig, postShort, postTall, side, sideAlt, sideTall, sideTallAlt, sideShort, sideAltShort,
				sideTallShort, sideTallAltShort);
	}
	...
	private void logWallBlockInternal(WallBlock block, String baseName, ResourceLocation texture, boolean postUsesTop,
			...
			ModelFile sideTall, ModelFile sideTallAlt, ModelFile sideShort, ModelFile sideAltShort,
			ModelFile sideTallShort, ModelFile sideTallAltShort) {
		this.logWallBlock(this.getMultipartBuilder(block),
				models().getBuilder(baseName + "_post_short").parent(postShort).texture("particle", texture)
						.texture("top", texture).texture("side", texture),
				models().getBuilder(baseName + "_post_tall").parent(postTall).texture("particle", texture)
						.texture("top", texture).texture("side", texture),
				models().getBuilder(baseName + "_side").parent(side).texture("particle", texture)
						.texture("top", texture).texture("side", texture),
				models().getBuilder(baseName + "_side_alt").parent(sideAlt).texture("particle", texture)
						.texture("top", texture).texture("side", texture),
				models().getBuilder(baseName + "_side_tall").parent(sideTall).texture("particle", texture)
						.texture("top", texture).texture("side", texture),
				models().getBuilder(baseName + "_side_tall_alt").parent(sideTallAlt).texture("particle", texture)
						.texture("top", texture).texture("side", texture));

		this.logWallBlockWithPost(this.getMultipartBuilder(block),
				models().getBuilder(baseName + "_post").parent(postBig).texture("particle", texture)
						.texture("top", postUsesTop ? (texture + "_top") : texture.toString()).texture("side", texture),
				models().getBuilder(baseName + "_side_short").parent(sideShort).texture("particle", texture)
						.texture("top", texture).texture("side", texture),
				models().getBuilder(baseName + "_side_alt_short").parent(sideAltShort).texture("particle", texture)
						.texture("top", texture).texture("side", texture),
				models().getBuilder(baseName + "_side_tall_short").parent(sideTallShort).texture("particle", texture)
						.texture("top", texture).texture("side", texture),
				models().getBuilder(baseName + "_side_tall_alt_short").parent(sideTallAltShort)
						.texture("particle", texture).texture("top", texture).texture("side", texture));
	}

	public void logWallBlock(MultiPartBlockStateBuilder builder, ModelFile postShort, ModelFile postTall,
			ModelFile side, ModelFile sideAlt, ModelFile sideTall, ModelFile sideTallAlt) {
		builder
				// Smaller post when West & East are both short while North & South being none
				.part().modelFile(postShort).addModel().nestedGroup().condition(WallBlock.UP, false)
				.condition(WallBlock.EAST_WALL, WallSide.LOW).condition(WallBlock.WEST_WALL, WallSide.LOW).end().end()
				// Taller thinner post when West & East are both tall while North & South being
				// none
				.part().modelFile(postTall).addModel().nestedGroup().condition(WallBlock.UP, false)
				.condition(WallBlock.EAST_WALL, WallSide.TALL).condition(WallBlock.WEST_WALL, WallSide.TALL).end().end()
				// Rotated small post when West & East are both none while North & South are
				// short
				.part().modelFile(postShort).rotationY(90).addModel().nestedGroup().condition(WallBlock.UP, false)
				.condition(WallBlock.EAST_WALL, WallSide.NONE).condition(WallBlock.NORTH_WALL, WallSide.LOW)
				.condition(WallBlock.WEST_WALL, WallSide.NONE).condition(WallBlock.SOUTH_WALL, WallSide.LOW).end().end()
				// Rotated small post when West & East are both none while North & South are
				// tall
				.part().modelFile(postTall).rotationY(90).addModel().nestedGroup().condition(WallBlock.UP, false)
				.condition(WallBlock.EAST_WALL, WallSide.NONE).condition(WallBlock.NORTH_WALL, WallSide.TALL)
				.condition(WallBlock.WEST_WALL, WallSide.NONE).condition(WallBlock.SOUTH_WALL, WallSide.TALL).end()
				.end();
	...
	public void logWallBlockWithPost(MultiPartBlockStateBuilder builder, ModelFile postBig, ModelFile side,
			ModelFile sideAlt, ModelFile sideTall, ModelFile sideTallAlt) {
		builder
				// Big post for connections, typically including angled
				.part().modelFile(postBig).addModel().condition(WallBlock.UP, true).end();
		WALL_PROPS.entrySet().stream().filter(e -> e.getKey().getAxis().isHorizontal()).forEach(e -> {
	...
	private void logWallSidePart(MultiPartBlockStateBuilder builder, ModelFile model, ModelFile modelAlt,
			Map.Entry<Direction, Property<WallSide>> entry, WallSide height, boolean hasPost) {
		int rotation = (((int) entry.getKey().toYRot()) + 180) % 360;
		builder.part().modelFile(rotation < 180 ? model : modelAlt).rotationY(rotation).addModel()
